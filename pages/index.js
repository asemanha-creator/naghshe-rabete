  ...
  betrayed: {
    1: {
      ...
      body: [
        ...
        { type: "technique", name: "۱. قانونِ «فقط امروز»", howTo: "به‌جایِ فکر به آینده، فقط بپرسید: «برایِ گذراندنِ امروز، به چه‌چیزی نیاز دارم؟» هر صبح ۲-۳ کارِ کوچک بنویسید (مثلاً: صبحانه بخورم، بیرون از تخت باشم، با یک نفر تماس بگیرم).", effect: "از غرق‌شدن در سوالاتِ بزرگ (آینده، تصمیم) جلوگیری می‌کند و روز را قابلِ‌مدیریت می‌کند.", more: "این تکنیک بر پایه‌ی اصلِ «تمرکز بر آنچه کنترل‌پذیر است» بنا شده. مغزِ آسیب‌دیده در برابرِ سوالاتِ بزرگ (آیا رابطه ادامه پیدا می‌کند؟) دچارِ فلج تصمیم می‌شود؛ کوچک‌کردنِ افق به «همین امروز» این فلج را می‌شکند. مثال: ماندا، در روزِ سوم، هرصبح فقط سه کارِ کوچک می‌نوشت و همین، روزهایِ اول را قابلِ‌تحمل کرد." },
        { type: "technique", name: "۲. لنگرِ حسی (Grounding)", howTo: "یک شیءِ فیزیکی (سنگ، دستبند، فنجانِ گرم) را ۳۰ ثانیه در دست بگیرید و فقط به بافت/وزن/دمایِ آن توجه کنید.", effect: "موجِ اضطراب یا تصویرِ ذهنیِ ناخواسته را در عرضِ کمتر از یک دقیقه کاهش می‌دهد.", more: "این تکنیک مغز را از حالتِ «بازپخشِ گذشته» به «واقعیتِ همین لحظه» منتقل می‌کند — دقیقاً برعکسِ مکانیزمِ آمیگدالِ فعال‌شده در ترومای حاد." },
        { type: "technique", name: "۳. تنفسِ ۴-۷-۸", howTo: "۴ ثانیه دَم از بینی، ۷ ثانیه نگه‌داشتن، ۸ ثانیه بازدَمِ آرام از دهان. سه دور.", effect: "ظرفِ کمتر از دو دقیقه، ضربانِ قلب و تنشِ عضلانی را پایین می‌آورد.", more: "این الگو مستقیماً سیستمِ عصبیِ پاراسمپاتیک (ترمزِ طبیعیِ بدن) را فعال می‌کند. قابلِ‌انجام در ماشین، دستشویی، یا هر جایی بدونِ جلبِ توجه." },
        { type: "technique", name: "۴. برنامه‌ی حداقلیِ بقایِ روزانه", howTo: "یک کارتِ ۴خطی بسازید: «۱) یک وعده غذا ۲) ۶ ساعت استراحت ۳) یک تماسِ حمایتی ۴) صفر تصمیمِ بزرگ.» هر شب تیک بزنید.", effect: "به بدن یادآوریِ نیازهایِ اولیه می‌دهد، بدونِ فشارِ «همه‌کاری را درست انجام‌دادن»." },
        { type: "technique", name: "۵. فعال‌سازیِ شبکه‌ی حمایتی", howTo: "همین امروز، ۲-۳ نفر را فهرست کنید که می‌توانید در لحظه‌ی سخت پیام بدهید — حتی فقط: «حالم خوب نیست، حرف می‌زنی؟»", effect: "تنهاییِ فیزیکی شدتِ آسیب را بیشتر می‌کند؛ این تکنیک از آن جلوگیری می‌کند." },
        { type: "technique", name: "۶. راهنمایِ خوابِ اورژانسی", howTo: "نورِ کم، بدونِ گوشی ۳۰ دقیقه قبل از خواب. اگر فکرِ مزاحم آمد، بنویسیدش. اگر بعد از ۲۰ دقیقه خواب نیامد، بلند شوید و در نورِ کم چیزی بخوانید.", effect: "اجبار به خوابیدن خودش اضطراب‌زاست؛ این روش فشار را کم می‌کند و خواب را تسهیل می‌کند." },
        { type: "technique", name: "۷. جعبه‌ی نگرانی", howTo: "هر فکرِ مزاحم را در یک جمله بنویسید و «داخلِ جعبه» بگذارید (فیزیکی یا نمادین): «این را برایِ بعد نگه می‌دارم.»", effect: "فکر را حذف نمی‌کند، اما آن را از جریانِ مداومِ ذهن به یک «قرارِ بعدی» تبدیل می‌کند." },
...
function Technique({ name, howTo, effect, more }) {
  const [showMore, setShowMore] = useState(false);
  return (
    <div style={{ marginBottom: 12, paddingBottom: 12, borderBottom: "1px solid #F0F4F7" }}>
      <p style={{ fontSize: 13, fontWeight: 800, color: "#1F2D3D", margin: "0 0 5px" }}>{name}</p>
    ...
    <div dir="rtl" style={{ ...FONT, minHeight: "100vh", background: "#EAF4FB", padding: "24px 16px" }}>
      ...
      <div style={{ maxWidth: 480, margin: "0 auto" }}>
        ...
        {screen === "start" && (
          <Card style={{ padding: 0, overflow: "hidden" }}>
            ...
            <p style={{ fontSize: 9.5, color: "#D3DEE4", marginTop: 14, textAlign: "center" }}>
              نسخه: ۲۰۲۶-۰۸-۲۷ / فرمتِ جدید: پاسخِ کوتاه+تاثیر برایِ هر تکنیک، با دکمه‌ی «بیشتر»
            </p>
            </div>
          </Card>
        )}
        ...
        {screen === "sessionReader" && viewingSession && (() => {
          ...
          // گروه‌بندیِ بلوک‌ها بر اساسِ تیترها — هر تیتر و محتوایِ زیرش، یک بخشِ کشویی می‌شود
          const sections = [];
          sess.body.forEach((block) => {
                  ...
                  <Collapsible key={si} title={sec.title}>
                    {sec.blocks.map((block, i) =>
                      block.type === "checklist" ? (
                        ...
                        <Technique key={i} name={block.name} howTo={block.howTo} effect={block.effect} more={block.more} />
                      ) : (
